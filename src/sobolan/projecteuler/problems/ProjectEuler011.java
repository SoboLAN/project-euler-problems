 ...
 * in the 20x20 grid?
 */
public class ProjectEuler011 extends AbstractExecutableProblem
{
    private final String FILE_PATH = "src/sobolan/projecteuler/problems/11.txt";

    private int[][] matrix;

    //directions for the diagonal checking
    private final int RIGHT = 1;
    private final int LEFT = 2;

    //these 2 constants are to avoid complicating the code because of zero-indexed arrays
    private final int MAX_ROW_INDEX = 20 - 1;
    private final int MAX_COL_INDEX = 20 - 1;

    @Override
    public String getResult()
    {
        this.matrix = this.readMatrixFromFile();
    ...
    {
        int[][] numbers = new int[this.MAX_ROW_INDEX + 1][this.MAX_COL_INDEX + 1];
    ...
    private int calculateLines()
    {
        ...
        for (int i = 0; i <= this.MAX_ROW_INDEX; i++) {
            for (int j = 0; j <= this.MAX_COL_INDEX - 3; j++) {
                int product = this.matrix[i][j] * this.matrix[i][j + 1] * this.matrix[i][j + 2] * this.matrix[i][j + 3];
    ...
    {
        int biggestProduct = 0;
        for (int i = 0; i <= this.MAX_ROW_INDEX - 3; i++) {
            for (int j = 0; j <= this.MAX_COL_INDEX; j++) {
                int product = this.matrix[i][j] * this.matrix[i + 1][j] * this.matrix[i + 2][j] * this.matrix[i + 3][j];
    ...
    /**
     * Determines if the number found at the row "row" and column "column" has at least 3 neighbors
     * when going down diagonally in the direction "direction"
     */
    private boolean hasDiagonalNeighborsBelow(int row, int column, int direction)
    {
        if (direction == this.RIGHT) {
            return (row + 3 <= this.MAX_ROW_INDEX && column + 3 <= this.MAX_COL_INDEX);
        }
        else if (direction == this.LEFT) {
            return (row + 3 <= this.MAX_ROW_INDEX && column - 3 >= 0);
        }
